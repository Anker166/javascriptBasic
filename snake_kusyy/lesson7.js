...
function move() {
    ...
    //  запускаем функцию отображения счета
    getScore();
    
    // проверка на границу поля, склеиваем границы
    if (new_unit == undefined){
        ...
        else if (coord_x == FIELD_SIZE_X - 1){
            coord_x = 0;
        }
...
function createBarrier() {
    var barrierCreated = 0;
    var barrierNum = parseInt(Math.random() * 9 + 1);

    while (barrierCreated < barrierNum) {
        // рандом
    ...
    if (gameIsRunning) {
        setTimeout(removeBarrier, 4000);
    }
}

// удаление барьеров
function removeBarrier() {
    var barriers = document.querySelectorAll('.barrier-unit');
    for (var elem of barriers) {
        var barrier_classes = elem.classList;
        elem.setAttribute('class', barrier_classes[0] + ' ' + barrier_classes[1]);
    }
    if (gameIsRunning) {
...
// проверка является ли клетка барьером
function isBarrier(unit) {
    var unit_classes = unit.getAttribute('class').split(' ');
    return unit_classes.includes('barrier-unit');
}
...
function getScore() {
    ...
    scoreTable.innerHTML = 'Ваш счет равен: ' + score;
}
